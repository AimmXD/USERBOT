...
from AimX.database.pmpermitdb import get_approved_users, pm_guard
import AimX.database.pmpermitdb as AimX
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await AimX.set_block_message(AimX.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await AimX.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await AimX.get_pm_settings()
    await AimX.allow_user(chat_id)
