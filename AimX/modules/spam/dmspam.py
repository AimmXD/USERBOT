...
async def dmraid(xspam: Client, e: Message):
      """ Module: Dm Raid """
      AimX = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
      if len(AimX) == 2:
          ok = await xspam.get_users(AimX[1])
          id = ok.id
          if int(id) in VERIFIED_USERS:
                text = f"Chal Chal baap Ko mat sikha"
                await e.reply_text(text)
          elif int(id) in SUDO_USERS:
                text = f"Abe Lawde that guy part of AɪᴍX."
                await e.reply_text(text)
          else:
              counts = int(AimX[0])
...
async def dmspam(spam: Client, e: Message):
      ...
      if len(AimX) == 2:
          ...
          if int(id) in VERIFIED_USERS:
                ...
                text = f"Abe Lawde that guy part of AimXbots."
                await e.reply_text(text)
          else:
              counts = int(text[0])
              msg = str(AimX[0])
